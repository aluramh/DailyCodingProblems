...
class Solution:
    ...
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        def helper(nums, low, high, threes):
            nonlocal cache
            ...
            while i < high:
                ...
                if three_sum == 0:
                    possible = [nums[low], nums[i], nums[high]]
                    if possible not in threes:
                        # Only store if not in cache already
                        key = f"{low}-{high}"
                        if key not in cache:
                            cache[key] = [possible]
                            threes.append(possible)

                # The sum is greater than 0, so reduce the "high" pointer to try out smaller numbers
                elif three_sum > 0:
                    # NOTE: - Return here because there is no point in checking out the other "n"s
                    if key not in cache:
                        helper(nums, low, high - 1, threes)

                # The sum is less than 0, so increase the "low" pointer to try out larger numbers
                elif three_sum < 0:
                    key = f"{low + 1}-{high}"
                    if key not in cache:
                        helper(nums, low + 1, high, threes)
        ...
        nums = sorted(nums)
        threes = []
        results = helper(nums, 0, len(nums) - 1, threes)
        return results
